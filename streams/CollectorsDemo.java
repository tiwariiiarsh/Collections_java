...
public class CollectorsDemo {
    public static void main(String[] args) {
        ...
        ArrayDeque<String> collect = names.stream().collect(Collectors.toCollection(() -> new ArrayDeque<>()));  //() -> new ArrayDeque<>() --> it is supplier lambda expression
//        Collectors.toCollection(...) --> Ye specify karta hai ki kaunsi collection chahiye
//        Unlike toList() or toSet(), yahan tum custom collection choose kar sakte ho
...
//         below both code give same  output
        System.out.println(words.stream().collect(Collectors.mapping(x -> x.toUpperCase(), Collectors.toList())));
        System.out.println(words.stream().map(x -> x.toUpperCase()).toList());
...
//       Ye code list ke words ka frequency count karta hai using toMap().
//Har word ko key aur initial value 1 milti hai, aur duplicate aane par (x, y) -> x + y se count increase hota hai.
//Final result ek map hota hai jisme har word ka total occurrence stored hota hai.
    }
}